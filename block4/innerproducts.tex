...
\beginedxproblem{Orthogonal in R^3}{\dpa3}
...
\beginedxproblem{Orthogonal or not? 2}{\dpa1}
...
\edXsolution{
In the first space $C[0,\pi]$, we calculate \[\langle f, g\rangle = \int_0^\pi \sin x \ dx = 2,\] 
so $f$ and $g$ are not orthogonal.  However, in the second space, we have
\[\langle f, h\rangle = \int_{-\pi}^\pi \sin x \ dx = 0,\]
so $f$ and $g$ {\emph{are}} orthogonal in $C[-\pi,\pi]$.  
}
