...
\edXsolution{If we align the tip of $w$ and the tail of $v$ to get $v+w$, we obtain $g$.  

\\
Now, $-w$ swaps the tip and tail of $w$, so to find $v-w$, we fix $v$, align the tip of $w$ with the tip of $v$ and draw a vector from the tail of $v$ to the tail of $w$.  That gives us $d$.  

\\
To find $w-v$, we repeat the above with the roles of $w$ and $v$ swapped, to give $a$.  Alternatively, we can note that $w-v=-(v-w)=-d=a.$  

\\
Finally, $-v$ is the vector with the same magnitude as $v$, but opposite direction to $v$.  This is $c.$}
...
\beginedxproblem{Vector Spaces}{\dpa1}
