...
\beginedxvertical{Row-Reduction Tools}
...
\doedxvideo{Solution Sets to Matrix-Vector Equations}{VreiYQU_GTA}
...
call the  {\keyb{\bf trivial solution.}}  If there are no solutions other than the trivial solution,
...
\input{solutionsetcheck.tex}

\edXabox{type="custom" cfn="spangrader" expect="<-1,1,1,0>;<7,-6,0,1>"}
...
\beginedxvertical{Solution Sets Summary}

\beginedxtext{Solution Sets Summary}



{\keya{\bf{Proposition.}}}  If $Ax = v$ is consistent, then the set of solutions to $Ax = v$ is a translation
...
\edXabox{type="multichoice" expect="For every vector $v\in \R^m$, the equation $Ax = v$ has at least one solution" options="For every vector $v\in \R^m$, the equation $Ax = v$ has exactly one solution","For every vector $v\in \R^m$, the equation $Ax = v$ has at least one solution","For every vector $v\in \R^m$, the equation $Ax = v$ has at most one solution"}
...
\edXsolution{ 
If there are fewer than 5 columns, then it is impossible for the columns of $A$ to span $\R^5$. 
In that case, 
...
The two cases in which there are fewer columns than rows are the answer.  
 }
...
\beginedxproblem{Consistent Matrix-Vector Equations}{\dpa3}
